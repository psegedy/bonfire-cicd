...
def _get_pod_logs(oc: OpenshiftClient, ns: str) -> None:
    ...
    pods_json = json.loads(oc.get.pods(namespace=ns, output="json").stdout)
    ...
    for pod, containers in pods_containers.items():
        for container in containers:
            current = oc.logs(pod, container=container, namespace=ns, _ignore_errors=True)
            previous = oc.logs(pod, container=container, namespace=ns, _ignore_errors=True)
            if current:
...
def _collect_k8s_artifacts(oc: OpenshiftClient, ns: str) -> None:
    ...
    events = oc.get.events(namespace=ns, sort_by=".lastTimestamp")
    all = oc.get.all(namespace=ns, output="yaml")
    clowdapp = oc.get.clowdapp(namespace=ns, output="yaml")
    clowdenv = oc.get.clowdenvironment(f"env-{ns}", output="yaml")
    cji = oc.get.clowdjobinvocation(namespace=ns, output="yaml")
